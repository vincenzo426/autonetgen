...
const RoleDistributionView = ({ roles }) => {
  ...
  const colors = {
    ...
    FIREWALL: "#DC2626", // NUOVO: red/crimson per firewall
  ...
  // Descrizioni per i ruoli (NUOVO: aggiunta descrizione firewall)
  const roleDescriptions = {
    CLIENT: "Client devices and workstations",
    SERVER: "General server systems",
    PLC_MODBUS: "MODBUS industrial controllers",
    PLC_S7COMM: "Siemens S7 industrial controllers",
    PLC_ETHERNET_IP: "EtherNet/IP industrial controllers",
    WEB_SERVER: "Web and HTTP servers",
    DATABASE_SERVER: "Database management systems",
    WEB_CLIENT: "Web browsing clients",
    GATEWAY: "Network gateways and routers",
    FIREWALL: "Security firewalls and filtering devices", // NUOVO
    DNS_SERVER: "Domain name resolution servers",
    MAIL_SERVER: "Email and messaging servers",
    SSH_SERVER: "SSH remote access servers",
    MQTT_BROKER: "IoT messaging brokers",
    UNKNOWN: "Unidentified or mixed-role devices",
  };
  ...
  const roleData = Object.entries(roles).map(([role, count]) => {
    ...
    return {
      ...
      description: roleDescriptions[role] || "Unknown role type",
    };
  });
